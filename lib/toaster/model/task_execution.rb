...
module Toaster
  class TaskExecution < MongoDBObject
    ...
    def initialize(task, state_before, state_after, state_changes, uuid = nil, start_time=nil)
      @task = task
    ...
    def reduce_and_set_state_after(state)
      ...
      @state_before = states_new[0]
      @state_after = states_new[1]
    end
    ...
    def self.find(criteria={}, preset_fields={})
      ...
      execs.each do |hash|
        ...
        if hash["state_changes"]
          obj.state_changes = StatePropertyChange.from_hash_array(hash["state_changes"])
        end
    ...
    def self.load_all_for_automation(auto)
      ...
      auto_runs = auto.get_runs().collect { |r| r.id }
