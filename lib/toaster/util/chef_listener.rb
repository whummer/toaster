...
module Toaster
  ...
  class ChefListener
    ...
    def self.get_task_from_sourcecode_line(resource, action, start_source_line)
      ...
      # get or create task
      task = Task.load(resource, action, sourcecode, sourcefile, sourceline)
      return task
    end
    ...
    def self.update_current_automation_info(run, ctx)
      ...
      run.automation = Automation.find_by_name_and_runlist(node_name, run_list)
      ...
      if !run.automation
        ...
        run.automation = Automation.load(node_name, task_list, params, run_list)
            ...
            if ChefListener.tasks_by_resource[resource]
              ...
              args.each do |c| 
                if c.respond_to?("source_line")
                  ...
                  # TODO: checked again, not sure if the statement above is true :/
                  break
                end
              end
